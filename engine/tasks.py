...
class PredictTask(Task):
    ...
    def __call__(self, *args, **kwargs):
        ...
        if not self.model:
            ...
            self.model = YOLOv8.YOLOv8(
                config.MODEL_PATH,
                conf_thres=config.SCORE_THRESHOLD,
...
@app.task(
    ...
    name="{query}.{task_name}".format(
        query=config.QUERY_NAME, task_name=config.ML_OBJECT_DETECTION_TASK_NAME
    ),
)
def object_detection_task(self, task_id: str, data: bytes):
    ...
    try:
        ...
        data["detection_draw_url"] = (
            config.ML_STORAGE_RESULTS_PATH
            + string_time
            + "/"
            + str(task_id)
            + config.ML_IMAGE_TYPE
        )
