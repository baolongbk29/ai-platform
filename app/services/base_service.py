...
class BaseService:
    ...
    async def get_by_id_after_check_is_deleted(self, model_id: int):
        return await self._repository.select_by_id_after_check_is_deleted(
            model_id=model_id
        )
    ...
    async def patch_after_check_user_token(
        self, model_id: int, dto: BaseModel, user_token: str
    ):
        return await self._repository.update_after_check_user_token(
            model_id=model_id, dto=dto, user_token=user_token
        )
    ...
    async def remove_by_id_after_check_user_token(self, model_id: int, user_token: str):
        return await self._repository.delete_by_id_after_check_user_token(
            model_id=model_id, user_token=user_token
        )

    async def soft_remove_by_id_after_check_user_token(
        self, model_id: int, user_token: str
    ):
        return await self._repository.soft_delete_by_id_after_check_user_token(
            model_id=model_id, user_token=user_token
        )
